...
def patch_scorer(monkeypatch, fake_client):
    ...
    def _factory(args):
        fc = fake_client(
            script={
                "MEMBER": [
                    "born 1809 hardin county"
                ],  # identical -> full concentration
...
def test_cli_member_scores_above_novel(patch_scorer, tmp_path, capsys):
    m = tmp_path / "m.txt"
    m.write_text("MEMBER passage")
    nvl = tmp_path / "n.txt"
    nvl.write_text("NOVEL passage")
...
def test_cli_calibrate_percentile(patch_scorer, tmp_path, capsys):
    ...
    suspect.write_text("MEMBER passage")
    clean1 = tmp_path / "c1.txt"
    clean1.write_text("NOVEL one")
    clean2 = tmp_path / "c2.txt"
    clean2.write_text("NOVEL two")
    rc = cli.main(
        ["--model", "m", "--json", "--calibrate", f"{clean1},{clean2}", str(suspect)]
    )
    data = json.loads(capsys.readouterr().out)
    assert rc == 0
